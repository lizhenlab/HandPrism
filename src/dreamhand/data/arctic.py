...
def _decode_images(image_root: Path, indices: Tensor, image_offset: int) -> Tensor:
    try:
    ...
    return torch.stack(frames, dim=1).float().div(127.5).sub(1.0)
...
def load_arctic_window(
    ...
    *,
    ...
    video = _decode_images(root / "data" / "images" / sequence / "0", indices, int(image_offset))
