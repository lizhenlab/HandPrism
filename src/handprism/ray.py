...
class MixedPnPOutput:
    ...
    joint_weights: Tensor | None = None
...
def mixed_pnp(
    ...
    *,
    ...
    joint_quality: Tensor | None = None,
    edge_quality: Tensor | None = None,
    ...
    enhanced = any(value is not None for value in (joint_quality, edge_quality, wrist_prior)) or bool(
        config.robust_iterations or config.depth_refine_fraction)
    if enhanced and config.architecture == CORE:
        raise ValueError("Core solver cannot use Fusion geometry refinements")
    for quality in (joint_quality, edge_quality):
        if quality is not None:
            if quality.shape != votes.shape:
                raise ValueError("joint quality must have one value per joint")
            # Detached, supervised quality cannot minimize pose loss by erasing
            # its weights. Keep a floor so early calibration cannot starve votes.
            weight = weight * torch.nan_to_num(quality.detach().float()).clamp(.05, 1.)
    ...
    for _ in range(config.robust_iterations):
        with torch.no_grad():
            uv, ok = projector((canonical_joints + translation.unsqueeze(-2)).detach())
            error = ((uv - anchors) * image_size[:, None, None, [1, 0]]).norm(dim=-1)
            robust = (config.robust_huber_px / error.clamp_min(config.robust_huber_px))
            robust = torch.where(ok & torch.isfinite(error), robust, torch.zeros_like(robust))
        weight = base_weight * robust
        denom = (weight * inv_z.square()).sum(-1).clamp_min(config.eps)
        xy = (weight[..., None] * inv_z[..., None] * (
            bearings - canonical_joints[..., :2] * inv_z[..., None])).sum(-2) / denom[..., None]
        translation = torch.cat((xy, depth[..., None]), -1)
    if config.depth_refine_fraction:
        # Weighted bearing equations with a depth prior. A bounded correction,
        # never an unconstrained focal/depth optimization on near-collinear rays.
        bx, by = bearings.unbind(-1)
        one, zero = torch.ones_like(bx), torch.zeros_like(bx)
        ax = torch.stack((one, zero, -bx), -1)
        ay = torch.stack((zero, one, -by), -1)
        a = torch.stack((ax, ay), -2).flatten(-3, -2)
        b = (bearings * canonical_joints[..., 2:] - canonical_joints[..., :2]).flatten(-2)
        w = weight.repeat_interleave(2, -1)
        atw = a.transpose(-1, -2) * w[..., None, :]
        gram = atw @ a
        rhs = (atw @ b[..., None]).squeeze(-1)
        ridge = torch.diag(gram.new_tensor([1e-4, 1e-4, .1]))
        reference = translation
        solution = torch.linalg.solve(gram + ridge, (rhs + reference @ ridge)[..., None]).squeeze(-1)
        span = depth * config.depth_refine_fraction
        refined_depth = solution[..., 2].clamp(depth - span, depth + span)
        inv_z = (canonical_joints[..., 2] + refined_depth[..., None]).clamp_min(config.min_depth_m).reciprocal()
        denom = (weight * inv_z.square()).sum(-1).clamp_min(config.eps)
        xy = (weight[..., None] * inv_z[..., None] * (
            bearings - canonical_joints[..., :2] * inv_z[..., None])).sum(-2) / denom[..., None]
        translation = torch.cat((xy, refined_depth[..., None]), -1)
    ...
    rms_denominator = weight.sum(-1).clamp_min(config.eps) if enhanced else vote_count.clamp_min(1)
    rms = ((residual_pixels.square() * weight).sum(-1) / rms_denominator).sqrt()
    ...
    if wrist_prior is not None:
        if wrist_prior.shape != translation.shape or wrist_log_scale is None:
            raise ValueError("independent wrist prior requires XYZ and log-scale")
        prior = wrist_prior.float() - wrist
        if not torch.isfinite(prior).all():
            raise ValueError("nonfinite independent wrist prior")
        # Compare metric uncertainty; the gating path itself is not a shortcut
        # for uncertainty heads to downweight their own supervised loss.
        with torch.no_grad():
            geometry_sigma = depth * rms / image_size.norm(dim=-1)[:, None, None]
            prior_variance = 2 * wrist_log_scale.float().exp().square().mean(-1)  # Laplace variance
            geometry_weight = (prior_variance / (prior_variance + geometry_sigma.square() + 1e-8))
            geometry_weight = torch.where(solved, geometry_weight, torch.zeros_like(geometry_weight))
        safe_translation = torch.where(solved[..., None], translation, prior)
        translation = geometry_weight[..., None] * safe_translation + (1 - geometry_weight[..., None]) * prior
    else:
    ...
    return MixedPnPOutput(
        ...
        joint_weights=weight if enhanced else None,
    )
