...
from .losses import HandPrismPrediction, HandPrismTarget, safe_annotations
...
def prediction_from_output(output: HandPrismOutput) -> HandPrismPrediction:
    return HandPrismPrediction(
        ...
        wrist_prior=output.decoder.wrist_prior,
        wrist_log_scale=output.decoder.wrist_log_scale,
        reliability_logits=output.decoder.reliability_logits,
        log_depth=output.decoder.log_depth,
    )
...
def target_from_batch(
    ...
    return HandPrismTarget(
        ...
        timestamps=batch.get("timestamps"), in_frame=batch.get("in_frame"),
        visibility_valid=batch.get("visibility_valid"), synthetic_occluded=batch.get("synthetic_occluded"),
    )


def scoring_batch(batch: dict, ray_height: int, ray_width: int) -> dict:
    """Finite GT placeholders before validation/test MANO and diagnostics.

    Only annotation fields are replaced in a shallow copy; image/calibration,
    masks, frame identities and original data remain unchanged. Never use this
    helper to repair model predictions or change which items are scored.
    """
    target = safe_annotations(target_from_batch(batch, ray_height, ray_width), masked_visibility=True)
    names = ("global_rotation", "articulation", "betas", "joints_root", "joints_camera",
             "translation", "joints_2d", "existence", "visibility")
    return {**batch, **{name: getattr(target, name) for name in names}}
