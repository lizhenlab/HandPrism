...
@dataclass
class HandPrismOutput:
    decoder: HandPrismDecoderOutput
    ray_field: Tensor
...
class HandPrismModel(nn.Module):
    def __init__(
        self,
        mano: ManoProvider,
        ...
        *,
        ...
        self.decoder = HandPrismDecoder(decoder_config, architecture=architecture, fusion_config=fusion_config)
    ...
    def forward(
        ...
        *,
        ...
        decoded = self.decoder(features, ray_field, target_frames, rgb_high=rgb_high,
                               roi_teacher=roi_teacher, roi_teacher_valid=roi_teacher_valid,
                               optimizer_step=optimizer_step, image_size=image_size)
    ...
    def _geometry(
        self,
        decoded: HandPrismDecoderOutput,
        ray_field: Tensor,
        *,
        target_frames: int,
        solver: str,
        ...
        pnp = mixed_pnp(
            ...
            self.solver_config,
            projector=projector,
            joint_quality=(decoded.reliability_logits.sigmoid()
                           if decoded.reliability_logits is not None else None),
            edge_quality=decoded.anchor_quality,
            wrist_prior=decoded.wrist_prior,
            wrist_log_scale=decoded.wrist_log_scale,
        )
        joints_camera = joints + pnp.translation.unsqueeze(-2)
        vertices_camera = vertices + pnp.translation.unsqueeze(-2)
        return HandPrismOutput(
            decoder=decoded,
            ray_field=ray_field,
