...
from .attention import AlternatingLayer, CrossAttention
from .fusion import FusionConfig, FusionRefinement
...
class HandPrismDecoderOutput:
    ...
    local_roi_valid: Tensor | None = None
    local_roi_bounds: Tensor | None = None
    anchor_quality: Tensor | None = None


class HandPrismDecoder(nn.Module):
    def __init__(self, config: DecoderConfig = DecoderConfig(), *, architecture: str,
                 fusion_config: FusionConfig = FusionConfig()) -> None:
        super().__init__()
        architecture_spec(architecture)
        self.architecture = architecture
        self.fusion_config = fusion_config
        if architecture == CORE and fusion_config.enabled:
            raise ValueError("Fusion refinements cannot be enabled for Core")
        ...
        # Optional heads must not consume the shared global/ray initialization stream.
        ...
        # Do not instantiate new parameters on Core/B0: preserves RNG and keys.
        self.final_norm = nn.LayerNorm(width) if fusion_config.final_readout else None
        self.final_memory_norm = nn.LayerNorm(width) if fusion_config.final_readout else None
        with torch.random.fork_rng(devices=[]):
            self.final_spatial = CrossAttention(width, config.heads) if fusion_config.final_readout else None
        with torch.random.fork_rng(devices=[]):
            self.refinement = FusionRefinement(width, fusion_config) if any((
                fusion_config.local_rgb, fusion_config.joint_mano,
                fusion_config.temporal_wrist, fusion_config.reliability)) else None
    ...
    def forward(
        ...
        ray_field: Tensor,
        target_frames: int,
        *, rgb_high: Tensor | None = None, roi_teacher: Tensor | None = None,
        roi_teacher_valid: Tensor | None = None, optimizer_step: int | None = None,
        ...
        if self.final_spatial is not None:
            hand_end = self.config.hand_queries
            joint_end = hand_end + self.config.joint_queries
            update, fresh_weights = self.final_spatial(
                self.final_norm(queries[:, :, hand_end:joint_end]).reshape(batch * latent_frames, -1, queries.shape[-1]),
                self.final_memory_norm(memory).flatten(0, 1))
            queries = torch.cat((queries[:, :, :hand_end],
                queries[:, :, hand_end:joint_end] + update.reshape(batch, latent_frames, -1, queries.shape[-1]),
                queries[:, :, joint_end:]), dim=2)
            weights = torch.cat((weights[:, :, :hand_end],
                fresh_weights.reshape(batch, latent_frames, -1, height * width),
                weights[:, :, joint_end:]), dim=2)
        decoded = self._readout(queries, weights, target_frames, height, width)
        if self.refinement is not None:
            decoded = self.refinement(decoded, rgb_high, roi_teacher, roi_teacher_valid, optimizer_step, image_size)
        if self.fusion_config.edge_quality:
            # A half-cell boundary is reachable by the expectation head. Quality
            # decreases across a FULL feature cell, unlike a fixed 2% margin.
            cell = decoded.anchors_2d.new_tensor([1 / width, 1 / height])
            distance = torch.minimum(decoded.anchors_2d, 1 - decoded.anchors_2d)
            decoded.anchor_quality = (distance / cell).clamp(0, 1).amin(-1)
        return decoded
