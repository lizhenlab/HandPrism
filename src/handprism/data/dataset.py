...
from .augmentation import AugmentationConfig, augment_sample
...
class HandPrismWindowDataset(Dataset[HandPrismSample]):
    def __init__(
        self,
        manifest: str | Path,
        *,
        ...
        self.frames = frames
        self.detail_long_side, self.extended_contract = detail_long_side, extended_contract
        self.augmentation = AugmentationConfig(**(augmentation or {}))
        if not 0 <= hard_window_fraction <= .5:
            raise ValueError("hard_window_fraction must be in [0,.5]; retain uniform exposure")
        self.hard_window_fraction = hard_window_fraction if training else 0.
        if self.hard_window_fraction and any(not isinstance(r.get("difficulty_windows"), list) for r in self.records):
            raise ValueError("hard sampling requires the frozen train-only difficulty index")
    ...
    def _start(self, record: dict[str, Any], draw_seed: int | None = None) -> int:
        ...
        if self.hard_window_fraction and draw_seed is not None:
            generator = torch.Generator().manual_seed(draw_seed)
            windows = record["difficulty_windows"]
            if windows and torch.rand((), generator=generator).item() < self.hard_window_fraction:
                strata = sorted({label for item in windows for label in item["strata"]})
                if strata:
                    label = strata[int(torch.randint(len(strata), (), generator=generator))]
                    candidates = [item for item in windows if label in item["strata"]]
                    ...
                    ranges = record.get("valid_ranges", [[
                        ...
                        int(record.get("start_max", int(record["num_frames"]) - self.frames)) + self.frames,
                    ]])
                    if not any(int(lo) <= start and start + self.frames <= int(hi) for lo, hi in ranges):
                        raise ValueError("difficulty index points outside valid training ranges")
                    return start
    ...
    def __getitem__(self, index: int | tuple[int, int]) -> HandPrismSample:
        ...
        extra = (dict(detail_long_side=self.detail_long_side, extended_contract=self.extended_contract)
                 if self.detail_long_side or self.extended_contract else {})
        if dataset == "arctic":
            sample = load_arctic_window(
                record["root"],
                record["sequence"],
                start,
                self.frames,
        ...
        if self.extended_contract:
            sample.synthetic_occluded = torch.zeros_like(sample.valid_joints_2d, dtype=torch.bool)
        if self.training and self.augmentation.enabled:
            sample = augment_sample(sample, self.augmentation,
                                    draw_seed if draw_seed is not None else int(torch.randint(2**31, ())))
        return sample


def collate_samples(samples: list[HandPrismSample]) -> dict[str, Any]:
    if not samples:
    ...
    for field in ("camera_parameters", "source_image_size", "gt_ray_field", "timestamps", "rgb_high",
                  "in_frame", "observed", "observed_valid", "visibility_valid", "synthetic_occluded"):
    ...
    result["timestamp_source"] = [sample.timestamp_source for sample in samples]
