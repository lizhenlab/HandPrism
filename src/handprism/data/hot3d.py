...
def load_hot3d_window(
    ...
    *,
    ...
    images, rotations, translations, annotations, detail = [], [], [], [], []
    if extended_contract:
        # Compare mask times to the actual RGB stream in the SAME time domain.
        rgb_times = set(provider.get_timestamps_ns(stream, TimeDomain.TIME_CODE))
        if any(timestamp not in rgb_times for timestamp in window):
            raise ValueError("HOT3D mask/RGB timestamp mismatch; CLOSEST could silently duplicate frames")
    for timestamp in window:
        if decode_rgb:
            ...
            native = image_data[0].to_numpy_array()
            images.append(_resize_native_image(native))
        if detail_long_side and decode_rgb:
            from PIL import Image
            upright = Image.fromarray(np.ascontiguousarray(np.rot90(native, k=3)))
            scale = min(1., detail_long_side / max(upright.size))
            upright = upright.resize(tuple(round(size * scale) for size in upright.size), Image.Resampling.BILINEAR)
            detail.append(torch.from_numpy(np.asarray(upright).copy()).permute(2, 0, 1))
    ...
    in_frame = visible.clone()
    visible &= torch.tensor([hand_visible.get(timestamp, False) for timestamp in window])[
        :, None, None
    ]
    video = (torch.stack(images, dim=1).float().div(127.5).sub(1.0) if decode_rgb
             else torch.zeros(3, frames, 1, 1))
    sample = HandPrismSample(
        dataset="hot3d",
        recording_id=root.name,
        frame_indices=torch.arange(start, start + frames),
        video=video,
        rgb_high=torch.stack(detail, dim=1) if detail else None,
        timestamps=torch.tensor(window, dtype=torch.float64) * 1e-9 if extended_contract else None,
        timestamp_source="hot3d_rgb_time_code_ns" if extended_contract else None,
        in_frame=in_frame if extended_contract else None,
        observed=torch.zeros_like(in_frame) if extended_contract else None,
        observed_valid=torch.zeros_like(in_frame) if extended_contract else None,
        # A stream-level 'some hand visible' flag cannot label each joint/hand
        # as unoccluded. Preserve unknowns instead of fabricating positives.
        visibility_valid=torch.ones_like(geometry["presence"]) if extended_contract else None,
        ...
        visibility=in_frame.any(-1) if extended_contract else visible.any(-1),
